...
api_id = 13193750;
api_hash = "eaf7e7a901a531a807604f0a8b41eeb";
...
class Bot(object):
    ...
    def start(self, limit=99):
        ...
        try:
            ...
            timer = {'Daily':0, 'Claim':0, 'Unlimited':0};
            for _ in range(limit):

                if time.time()>=timer['Daily']:
                    self.send_message("🎉 Daily Giveaway");
                    messages = self.get_messages(limit=1)[0];
                ...
                if time.time()>=timer['Claim']:
                    self.send_message("💙 Claim Faucet Trx");
                    messages = self.get_messages(limit=1)[0];
                    if "❌ Email Not set" in messages.message:
                        self.send_message("💼 Set Email");
                        self.click(messages, 0);
                        while True:
                            Email = input("Input Your Faucetpay.io Email: ");
                            if Email:
                                self.send_message(Email);
                                message = self.get_messages(limit=1)[0].message
                ...
                if time.time()>=timer['Unlimited']:
                    self.send_message("😈 Unlimited Faucet");
                    timer['Unlimited'] = time.time()+120;
                    messages = self.get_messages(limit=3);
    ...
    def get_messages(self, limit=1):
        time.sleep(1.75);
        return self.client.get_messages(self.channel_username, limit=limit);
    ...
    def click(self, messages, i):
        ...
        time.sleep(1.25);
        messages = self.get_messages(limit=2);
        if "c7761ee5e3d2bd10f427602774253357dbd2bd4f" in messages[1].message:
            console_print(messages[1].message.split('\n')[0]);
        elif "c7761ee5e3d2bd10f427602774253357dbd2bd4f" in messages[0].message:
            console_print(messages[0].message.split('\n')[0]);
